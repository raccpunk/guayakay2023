...
@Component({
  selector: 'app-rcobrospagos',
  templateUrl: './rcobrospagos.component.html',
  styleUrls: ['./rcobrospagos.component.scss']
})
export class RCobrospagosComponent {
  ...
  vercobrosselect(id: number) {
   ...
   generarExcel() {
    const excelData = [];
    excelData.push(['FECHA ACTUALIZACION', 'FOLIO', 'USUARIO', 'CLIENTE', 'MÉTODO DE COBRO', 'FORMA DE PAGO', 'IVA %', '$ IVA', 'SUB TOTAL', 'TOTAL', 'PAGADO' , 'ESTATUS']);

    this.historial_lista.forEach((historial: { updatedAt: any; idcobro: any; tblUsuario: { nombre: any; } | null; tblCliente: { nombre: any; }; tipo_cobro: any; tipo_pago: any; iva_porcentaje: any; iva_numero: any; monto_total: any; pagado: any; estatus: any; }) => {
      // Formatear la fecha en formato de fecha corta de Excel
      const fechaCortaExcel = new Date(historial.updatedAt);
